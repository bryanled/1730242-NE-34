...
if (!function_exists('almighty_get_option')):
 ...
 */
function almighty_get_option($key) {
 ...
 */
function almighty_customize_register($wp_customize) {
	...
	if (isset($wp_customize->selective_refresh)) {
		$wp_customize->selective_refresh->add_partial('blogname', array(
				...
				'render_callback' => 'almighty_customize_partial_blogname',
			));
		$wp_customize->selective_refresh->add_partial('blogdescription', array(
				'selector'        => '.site-description',
				'render_callback' => 'almighty_customize_partial_blogdescription',
			));
	}

    $wp_customize->register_section_type( 'Almighty_Customize_Section_Upsell' );
    // Register sections.
    $wp_customize->add_section(new Almighty_Customize_Section_Upsell(
            $wp_customize,
            'theme_upsell',
            array(
                'title'    => esc_html__( 'View Pro Version', 'almighty' ),
                'pro_text' => esc_html__( 'Upgrade', 'almighty' ),
                'pro_url'  => 'https://unitedtheme.com/themes/almighty-plus/',
                'priority'  => 1,
            )
        )
    );
...
add_action('customize_register', 'almighty_customize_register');
 ...
 * @since 1.0.0
 */
function almighty_customize_preview_js() {

	wp_enqueue_script('almighty_customizer', get_template_directory_uri().'/js/customizer.js', array('customize-preview'), '20130508', true);

}

add_action('customize_preview_init', 'almighty_customize_preview_js');

function almighty_upsell_js() {
    wp_enqueue_script( 'almighty_customize_controls', get_template_directory_uri() . '/assets/js/upsell.js', array( 'customize-controls' ) );
}
add_action( 'customize_controls_enqueue_scripts', 'almighty_upsell_js',0 );